...
def plot_corr_diff(y: np.ndarray, y_hat: np.ndarray, plot_diff=False, *args, **kwargs):
    ...
    if plot_diff:
        diff = abs(np.subtract(y_corr, y_hat_corr))
        sns.set(style="white")
    ...
    for i, label in enumerate(titles):
        ...
        ax[i].set_yticklabels(y.columns.values, rotation='horizontal')
        ax[i].set_xticks(list(np.arange(0.5, y_corr.shape[0] + 0.5, 1)))
